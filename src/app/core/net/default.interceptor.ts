...
const TIME = { nzDuration: 500 };
const ERRORTIME = { nzDuration: 1000 };
...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private handleData(ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    switch (ev.status) {
      case 200:
        ...
        switch (res.code) {
          case 10000:
            this.notification.success(`操作码:${res.code}`, `返回信息:${isNull(res.msg) ? `${responsetext}` : res.msg}`, TIME);
            break;
          case 10001:
            this.notification.info(`操作码:${res.code}`, `返回信息:${isNull(res.msg) ? `${responsetext}` : res.msg}`, ERRORTIME);
            break;
          case 10004:
            this.notification.error(`操作码:${res.code}`, `返回信息:${isNull(res.msg) ? `${responsetext}` : res.msg}`, ERRORTIME);
            break;
        }
        break;
      case 401:
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    //token;
    const token = this.tokenSrv.get().token;
    if (!url.endsWith('login')) {//非登录接口则自动加头部token验证
      newReq = req.clone(
        {
          url,
          setHeaders: {
